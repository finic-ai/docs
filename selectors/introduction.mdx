...
---
<Note>**The Finic Selector Service is currently in development.** You can use the Finic SDK to [generate new selectors locally](selectors/generating-selectors#using-the-finic-selector-generator), but will not be able to sync them to Finic Selector Service yet.</Note>
...
Finic Selector Service is an improvement over this process by fully managing the selector lifecycle on your behalf. It notifies you when selectors break and can even fixing them automatically without developer intervention or code changes.
...
```
If the `regenerate` flag is set, FSS will first attempt to locate the element with the stored selector. If it isn't found, it will call an LLM to generate a new selector based on the current page and cached HTML from previous successful runs.
